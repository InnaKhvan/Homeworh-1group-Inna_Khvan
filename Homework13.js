async function firstCostSum() {
    const [food, ingPrice] = await Promise.all([
        fetch("./food.json").then((response) => response.json()),
        fetch("../ingPrice.json").then(response => response.json())
    ]);
    setTimeout(function () {
...
// console.log("Запишите в каждое блюдо себестоимость, используя map и функцию из первого задания.")
// let costSum1 = food.map(function (foodItem) {
//     let costPrice = foodItem.ingredients.reduce(function (sum, item) {
//         return sum + ingPrice[item]
//     }, 0)
//     foodItem.cost = costPrice
